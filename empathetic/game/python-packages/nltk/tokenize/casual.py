...
1. The tuple rePS defines a list of regular expression
   strings.

2. The rePS strings are put, in order, into a compiled
...
import re  # https://github.com/nltk/nltk/issues/2409
...
# appears first in the final re (since it can contain whitespace).
...
# For re simplicity, include all possible enclosed letter pairs,
...
# Future regional flag support may be handled with the re for
...
# re for recognizing phone numbers:
PHONE_re = r"""
    (?:
      (?:            # (international)
        \+?[01]
...
# Take the main components and add a phone re as the second parameter
rePS_PHONE = (rePS[0], PHONE_re, *rePS[1:])

######################################################################
# TweetTokenizer.WORD_RE and TweetTokenizer.PHONE_WORD_RE represent
# the core tokenizing rees. They are compiled lazily.

# WORD_RE performs poorly on these patterns:
HANG_RE = re.compile(r"([^a-zA-Z0-9])\1{3,}")

# The emoticon string gets its own re so that we can preserve case for
# them as needed:
...
class TweetTokenizer(TokenizerI):
    ...
    # which are the core tokenizing rees.
    _WORD_RE = None
    _PHONE_WORD_RE = None
    ...
    def WORD_RE(self) -> "re.Pattern":
        """Core TweetTokenizer re"""
        # Compiles the re for this and all future instantiations of TweetTokenizer.
    ...
    @property
    def PHONE_WORD_RE(self) -> "re.Pattern":
        """Secondary core TweetTokenizer re"""
        # Compiles the re for this and all future instantiations of TweetTokenizer.
        if not type(self)._PHONE_WORD_RE:
            type(self)._PHONE_WORD_RE = re.compile(
                f"({'|'.join(rePS_PHONE)})",
                re.VERBOSE | re.I | re.UNICODE,
            )
